...
module FSharp.ProvidedTypes.AddStaticOverSimplifiedAlgebra
...
let AppendSimpleStaticParameter (name: string, ty: Type, defaultValue: option<'T>) (arr: ISimpleStaticParameter[]) =
    Array.append([| {new ISimpleStaticParameter with
...
let AddStaticParam(name: string, ty: Type, defaultValue: option<'T>) (tp: ISimpleTypeProvider) = 
