...
let AddStaticParameter(name: string, ty: Type, defaultValue, tp: ITypeProvider) = 
    ...
    let TxTypeProviderDefinition (inp: ITypeProvider) = 
        ...
        { new ITypeProvider with 
            ...
            override __.GetStaticParameters(typeWithoutArguments) = 
                Array.append
                    (inp.GetStaticParameters(unwrap typeWithoutArguments))
                    [| ProviderImplementation.ProvidedTypes.ProvidedStaticParameter(name, ty, defaultValue) |]
